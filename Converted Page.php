 ...
 * - game_id  = WA7 (Washington Match 4)
 * - main balls = 4
 * - main number range = 01..24
 * - no bonus / special ball
 */
...
$gameMainMin   = 1;
$gameMainMax   = 24;
$mainBallCols  = ['first', 'second', 'third', 'fourth'];
$hasBonus      = false;
$bonusCol      = '';
$bonusMin      = 0;
$bonusMax      = 0;
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData('description', 'Verified draw results, frequency analysis, hot and quiet numbers, and recency data for ' . $stateName . ' ' . $gName . '. Explore draw history, active number patterns, and full distribution for Pick 4 numbers 01–24.');
}
...
function leFetchRecentDraws(
    ...
    string $gameId,
    int $limit,
    array $ballCols = ['first', 'second', 'third', 'fourth'],
    string $bonusColName = ''
): array {
    $selectCols = [
        $db->quoteName('id'),
    ...
    if ($bonusColName !== '') {
        $selectCols[] = $db->quoteName($bonusColName);
    }
...
function leGetPreviousOccurrenceDate(
    ...
    string $drawDate,
    string $ball,
    array $mainCols = ['first', 'second', 'third', 'fourth'],
    string $bonusColName = '',
    bool $isBonus = false
): ?string {
    if ($ball === '') {
    ...
    if ($isBonus && $bonusColName !== '') {
        $query->where($db->quoteName($bonusColName) . ' = ' . $db->quote($ball));
    } else {
        $conditions = [];
...
// ─── Data Fetch ───────────────────────────────────────────────────────────────
$latestRows = leFetchRecentDraws($db, (string) $dbCol, $gameId, 1, $mainBallCols);
...
$latestMainBalls = [$p1, $p2, $p3, $p4];
$logo            = (isset($stateAbrev, $gName)) ? leResolveLogo((string) $stateAbrev, (string) $gName) : ['exists' => false, 'url' => ''];

$rowsMain = leFetchRecentDraws($db, (string) $dbCol, $gameId, $nodCurrentMain, $mainBallCols);
...
// ─── Window Comparison ────────────────────────────────────────────────────────
$window50  = leFetchRecentDraws($db, (string) $dbCol, $gameId, 50,  $mainBallCols);
$window300 = leFetchRecentDraws($db, (string) $dbCol, $gameId, 300, $mainBallCols);
...
// ─── Draw History for Current Draw Context ────────────────────────────────────
$drawHistoryRows = [];
...
$jsonLdWebPage = [
    ...
    '@type'       => 'WebPage',
    'name'        => (isset($gName) ? (string) $gName : '') . ' Results Intelligence — ' . (isset($stateName) ? (string) $stateName : '') . ' | LottoExpert.net',
    'description' => 'Verified draw results, frequency analysis, and historical number behavior for ' . (isset($gName) ? (string) $gName : '') . ' in ' . (isset($stateName) ? (string) $stateName : '') . '. Pick 4 numbers 01–24 analyzed across configurable draw windows.',
    'url'         => $canonicalNoQuery,
    'inLanguage'  => 'en',
    'publisher'   => ['@type' => 'Organization', 'name' => 'LottoExpert.net', 'url' => 'https://lottoexpert.net'],
];

$jsonLdDataset = [
    '@context'         => 'https://schema.org',
    '@type'            => 'Dataset',
    'name'             => (isset($gName) ? (string) $gName : '') . ' Draw Frequency Data',
    'description'      => 'Number frequency, recency, active and quiet number data for ' . (isset($gName) ? (string) $gName : '') . ' draws. Covers the last ' . (int) $nodCurrentMain . ' drawings with full distribution for Pick 4 numbers 01–24.',
    'creator'          => ['@type' => 'Organization', 'name' => 'LottoExpert.net'],
    'temporalCoverage' => $drawDate !== '' ? substr($drawDate, 0, 4) : '',
    'variableMeasured' => ['draw frequency', 'recency', 'hot numbers', 'cold numbers'],
];
?>
<style>
:root{
...
<script type="application/ld+json"><?php echo json_encode($jsonLdWebPage, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?></script>
<script type="application/ld+json"><?php echo json_encode($jsonLdDataset, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?></script>
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=WA7">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=WA7">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/washington-match-4-skip-and-hit-analysis">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=WA7">MCMC Markov Analysis</a>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active, which are quiet, and how the current draw relates to recent history. All data applies to the <?php echo (int) $gameMainBalls; ?> main numbers drawn from the 01&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?> pool.
        </p>
      </div>
    </div>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This section makes the current draw easier to interpret. It shows how recently each latest number was previously seen and how the perspective shifts when you compare a short recent window with a broader historical one. Each of the <?php echo (int) $gameMainBalls; ?> main numbers drawn is examined individually.
        </p>
      </div>
    </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values 01&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?>. The right panel shows how recently each number last appeared across the same window. This is the full analytical view for the Pick 4 main pool.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. Each bar represents a specific number from the 01&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?> pool — the height reflects how many times it appeared within the current <?php echo (int) $nodCurrentMain; ?>-draw window.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the table for exact counts, recency, and personal tracking of the <?php echo (int) $gameMainBalls; ?> main numbers drawn from the 01&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?> pool. Quick filters help narrow the view without losing access to the full dataset.
        </p>
      </div>
    </div>
  ...
  <section id="tools" class="skai-section" aria-labelledby="tools-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          The results page establishes context. These tools take that context into deeper modeling and structured exploration for <?php echo htmlspecialchars((string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?>.
        </p>
      </div>
    </div>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">
              Best next step for a broader multi-signal view. Use this after reviewing frequency and recency to move into the main SKAI intelligence workflow for <?php echo htmlspecialchars((string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?>.
            </p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=WA7">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=WA7">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/washington-match-4-skip-and-hit-analysis">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=WA7">MCMC Markov Analysis</a>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This page is designed to help users understand recent and historical behavior of <?php echo htmlspecialchars((string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?> more clearly, not to imply certainty about future outcomes.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing data for <?php echo htmlspecialchars((string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?> can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. <strong>Frequency</strong> measures how often a number appeared within the selected draw window. <strong>Recency</strong> measures how many draws have passed since a number last appeared. <strong>Active numbers</strong> are those with the highest frequency counts. <strong>Quiet numbers</strong> are those with the largest gap since their last appearance. The purpose of this page is to make the recent behavior of the game easier to understand, compare, and carry into deeper SKAI analysis. All <?php echo (int) $gameMainBalls; ?> main numbers are drawn from a pool of 01&ndash;<?php echo str_pad((string) $gameMainMax, 2, '0', STR_PAD_LEFT); ?>.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function safeCanvas(id) {
    ...
    if (parent && parent.offsetWidth === 0) { return null; }
    return el;
  }
  ...
  function renderCharts() {
    ...
    var topActiveCanvas = safeCanvas('topActiveChart');
    var quietCanvas     = safeCanvas('quietChart');
    var fullMainCanvas  = safeCanvas('fullMainChart');
    var recencyCanvas   = safeCanvas('recencyChart');

    if (topActiveCanvas && chartData.topActiveLabels.length) {
    ...
    if (quietCanvas && chartData.quietLabels.length) {
    ...
    if (fullMainCanvas && chartData.mainLabels.length) {
    ...
    if (recencyCanvas && chartData.mainLabels.length) {
...
<?php echo HTMLHelper::_('content.prepare', '{loadposition Pick4Wheels}'); ?>
